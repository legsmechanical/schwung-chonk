#!/usr/bin/env bash
# Install the CHONK module to Move over the USB-ethernet tether (or WiFi).
# Set MOVE_HOST to override (default ableton@move.local).
#
# A dsp.so swap loads fresh on the next instantiation; if CHONK is loaded,
...
echo "=== Installing CHONK Module -> $MOVE_HOST ==="
