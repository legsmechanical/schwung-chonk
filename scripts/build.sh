#!/usr/bin/env bash
# Build the CHONK module for Schwung (Move, ARM64).
#
...
if [ -z "$CROSS_PREFIX" ] && [ ! -f "/.dockerenv" ]; then
    echo "=== CHONK Module Build (via Docker) ==="
    ...
    if command -v node >/dev/null 2>&1; then
        # The host reads module.json's ui_hierarchy at synth load to build the
        # slot param table custom knobs resolve against; a stale copy is a set
        # of silently dead knob destinations.
        node "$REPO_ROOT/tools/gen_module_json.mjs" || exit 1
        # The factory bank is indexed by PARAMS[] position, so it must be
        # regenerated whenever a parameter is added or moved — otherwise every
        # preset past the insertion point loads into the wrong slot, and
...
# -Wno-comment: the generated Faust headers and our own block comments contain
#   '/*' inside comments (file globs in the provenance headers).
# -ffast-math is NOT used: the waveguide's stability depends on ordinary IEEE
#   rounding, and a denormal flush that changes the bounce term changes the
#   decay. If CPU becomes a problem, measure before reaching for it.
echo "Compiling DSP plugin..."
${CROSS_PREFIX}g++ -g -O3 -shared -fPIC -std=c++14 -Wno-comment \
    src/dsp/chonk_plugin.cpp \
    -o build/dsp.so \
    -Isrc/dsp \
    -lm
